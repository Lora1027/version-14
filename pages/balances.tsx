...
export default function Balances(){
  ...
  const [form,setForm] = useState<{kind:'capital'|'cash'|'gcash'|'bank';label:string;amount:number}>({
    kind:'capital', label:'', amount:0
  })

  async function load(){
    const { data, error } = await supabase.from('balances').select('*').order('kind',{ascending:true})
  ...
  async function updateRow(b:Bal){
    const { error } = await supabase.from('balances')
      ...
      <div className="container">
        ...
        <div className="card">
          ...
          <table>
            <thead>
              <tr>
                <th>Kind</th><th>Label</th><th>Amount</th><th></th>
              </tr>
            </thead>
            <tbody>
              {rows.map(r=>(
